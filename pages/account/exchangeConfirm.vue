			...
			<view class="info-model" v-if="exchangeInfo">
				...
				<view class="info-content-item flex-between">
					...
					<view class="info-content-account">
						<view class="info-account-name">{{exchangeInfo.type === '买入'?exchangeInfo.getAccountInfo.name : exchangeInfo.setAccountInfo.name}}</view>
						<view class="info-account-text">{{exchangeInfo.type === '买入'?exchangeInfo.getAccountInfo.accountNumber : exchangeInfo.setAccountInfo.accountNumber}}</view>
					</view>
				</view>
				<view class="info-content-item flex-between">
					<view class="info-content-title">
					...
					<view class="info-content-account">
						<view class="info-account-name">{{exchangeInfo.type === '买入'?exchangeInfo.setAccountInfo.name : exchangeInfo.getAccountInfo.name}}</view>
						<view class="info-account-text">{{exchangeInfo.type === '买入'?exchangeInfo.setAccountInfo.accountNumber : exchangeInfo.getAccountInfo.accountNumber}}</view>
					</view>
				</view>
			</view>
			<view class="info-model" v-if="exchangeInfo">
				...
				<view class="info-content-item flex-between" style="align-items: center;">
					...
					<view class="info-content-account">
						<view class="info-account-name red" :class="{green: exchangeInfo.type === '买入'}">{{exchangeInfo.setCurrencyInfo.name}} <text class="money">{{ exchangeInfo.setValue }}</text></view>
					</view>
				</view>
				<view class="info-content-item flex-between" style="align-items: center;margin-top: -0rpx;">
					...
					<view class="info-content-account">
						<view class="info-account-name red" :class="{green: exchangeInfo.type === '卖出'}">{{exchangeInfo.getCurrencyInfo.name}} <text class="money">{{ exchangeInfo.getValue }}</text></view>
					</view>
				</view>
...
export default {
	...
	mounted(){
		const exchangeInfo = localStorage.getItem('exchangeInfo')
		if(exchangeInfo){
			this.exchangeInfo = JSON.parse(exchangeInfo)
		}
	},
